...
export default function Page1() {
    ...
    const filteredapp = appsList.filter((app) => {
        const inputsearch = app.appName.toLowerCase().includes(searchInput.toLowerCase());
        const isCategoryMatch = app.category === activeTab;
        return searchInput === '' ? isCategoryMatch : inputsearch && isCategoryMatch;
        ...
        <div className='app-Container'>
            <div className='appparent'>
                ...
                <div className='tab-list'>
                    <input type="search" className='searchinput' onChange={(e) => setSearchInput(e.target.value)} value={searchInput} />
                    <img src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png " alt="search icon" className='search-icon ' />
                    <div className='input-container'>
                        {tabs.map((tab) => (
                            <button key={tab.tabId} onClick={() => handleTabClick(tab.tabId)} className={activeTab === tab.tabId ? 'active' : ''}>
                                {tab.displayText}
                            </button>
                        ))}
                    </div>
